...
pub struct StealthProfile {
    ...
    /// `<script src>` loads. When true, Obscura will refuse to
...
impl StealthProfile {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        // actually emit. `transport` picks the stack from browser_name +
        ...
        let expected_tls = crate::transport::expected_emulation(self);
...
mod tests {
    ...
    fn presets_declare_the_tls_stack_they_emit() {
        ...
        for (name, p) in presets {
            assert_eq!(
                p.tls_impersonate,
                crate::transport::expected_emulation(&p),
